...
for src_fname in listdir("./include/tinyspv/spirv/unified1/instrs"):
    HEADER = []
    OPERANDS = defaultdict(list)
    with open(f"./include/tinyspv/spirv/unified1/instrs/{src_fname}") as f:
        lines = f.readlines()
    ...
    with open(f"./include/tinyspv/spirv/unified1/serdes/{src_fname}", "w") as f:
        f.writelines(HEADER)
        f.write("#pragma once\n")
        f.write(f"#include \"tinyspv/spirv/unified1/instrs/{src_fname}\"\n")
        ...
        f.write("T deserialize_instr(const Instruction& it);\n\n")
